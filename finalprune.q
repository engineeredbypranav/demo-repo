/ rte.q - Real Time Engine (High Performance Rolling Window)

/ 0. Map .u.upd
.u.upd:upd;

/ --- CONFIG ---
/ 1. Pruning Window: How much history to keep in memory?
/ We keep 5 minutes buffer to be safe, even though calc uses 1 min.
KEEP_WINDOW: 00:05:00.000;

/ 2. Timer Frequency
/ Run cleanup and calc every 1 second (1000ms).
/ Because the table is now small, this will be instant.
\t 1000

/ --- TIMER LOGIC ---
.z.ts:{ 
    / A. PRUNE (Garbage Collection)
    / Remove data older than our window. This keeps the process fast forever.
    / Since data is time-ordered, this is a fast head-truncate operation.
...
getBidAskAvg:{[st;et;granularity;s]
    ...
    / Select raw data
    / This is now fast because rteData is kept small
...
/ 4. Upd Function (High Performance)
upd:{[t;x]
    @[{
        typ: type x;
        sourceTable: ();
...
runCalc:{
    @[{
        ...
        / Get Symbols (Fast on small table)
...
-1 "RTE Ready. Optimized Mode (Pruning Enabled).";
